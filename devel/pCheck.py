...
def phoneScanner(filename):
    ...
    if int(ans) == 1:
        ...
        phoneChecker2 = re.compile(r'\W\d{11}')
        phoneChecker3 = re.compile(r'(\d{10})(\W)')
        phoneChecker4 = re.compile(r'\(\d{3}\)\s\d{3}-\d{4}')
        mo1 = phoneChecker1.findall(textContent)
        mo2 = phoneChecker2.findall(textContent)
        mo3 = phoneChecker3.findall(textContent)
        mo4 = phoneChecker4.findall(textContent)
        ...
        print('Form four phone numbers found: ' + str(mo4))
    elif int(ans) == 2:
        ...
        phoneChecker1 = re.compile(r'\d{3}-\d{3}-\d{4}') #Regexes 1-4
        phoneChecker2 = re.compile(r'\W\d{11}')
        phoneChecker3 = re.compile(r'\d{10}')
        phoneChecker4 = re.compile(r'\(\d{3}\)\s\d{3}-\d{4}')
        pages = 0
        while pages < textContent.numPages: #Uses a loop that starts with page 0, and keeps going until every page is gotten.
            ...
            mo4 = phoneChecker4.findall(textPage.extractText())
            pages = pages + 1
